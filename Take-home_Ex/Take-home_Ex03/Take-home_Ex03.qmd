...
```{r}
pacman::p_load(tidyverse,SmartEDA,sf,plotly,tmap,terra,gstat)
```
:::
...
```{r}
# Spatial join to associate weather data points with subzones
weather_subzones <- st_join(weather_sf, mpsz2019, join = st_intersects)
```
...
tm_shape(mpsz2019) +
  tm_borders() +
tm_shape(weather_apr2021) +
  tm_dots(fill = 'MonthlyRainfall')
...
# **6 Spatial Interpolation: gstat method**
...
## **6.1 Inverse Distance Weighted (IDW)**

```{r}
res <- gstat(formula = MonthlyRainfall ~ 1, 
             locations = weather_apr2021, 
             nmax = 10,
             set = list(idp = 0))
